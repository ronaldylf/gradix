...
export function ValidadeChairCode(code: string, label: string): IValidatedChairData | Error {
    if (code==='' || label==='') { // verificar se especificou o código
        return new Error("Cadeira incompleta")
    }

    const letras = code.split('').filter(c => ['M', 'T', 'N'].indexOf(c) !== (-1))
    
    if (letras.length !== 1) { // verificar se especificou apenas 1 turno
    ...
    if (!soNumeroRegex.test(ladoEsquerdo) || !soNumeroRegex.test(ladoDireito)) { // verifica se contém somente números nos dias e horários
    ...
    if (dias_validos.length!==nums_dias.length || horarios_validos.length!==nums_horarios.length) { // verifica se os dias e horários são somente números
    ...
    const rows = nums_horarios.map((value) => {
        if (turno==='T') { return (Number(value)-1+6) }
        if (turno==='N') { return (Number(value)-1+12) }
