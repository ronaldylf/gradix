...
function Amount() {
    ...
    useEffect(() => {
        ...
        timeTable.data.forEach((row) => {
            row.forEach((slot) => {
                ...
                if (slot.row !== -1 && alreadyAddedLabels.indexOf(slot.childChair.label)===(-1)) {
        ...
        let newAmounts = {
            ...
            amountNotRequired: (allFilledSlots.length - requiredFilledSlots.length)
        }
    ...
    <div className="flex gap-2 text-2xl rounded-2xl border-2 p-3 px-5 w-max">
        <Label className="cursor-text">Optativas: {amounts.amountNotRequired}</Label>
        <Label className="cursor-text">|</Label>
        <Label className="cursor-text">Obrigatórias: {amounts.amountRequired}</Label>
        <Label className="cursor-text">|</Label>
        <Label className="cursor-text underline"><b>Total: {amounts.amountTotal}</b></Label>
    </div>
    )
}
